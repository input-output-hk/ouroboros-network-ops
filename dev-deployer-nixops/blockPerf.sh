...
CONFIG=$(systemctl status | grep -oP '/nix/store/\w+-config-0-0.json') 
...
MQTT_CA="/run/keys/iog-network-team-mqtt"
MQTT_CERT="/run/keys/iog-network-team-certificate"
MQTT_KEY="/run/keys/iog-network-team-private"

MQTT_HOST="a12j2zhynbsgdv-ats.iot.eu-central-1.amazonaws.com"
MQTT_USER="iog-network-team"
MQTT_PORT="8883"
RELAY_PUBLIC_IP=${1}
...
do
  ...
  if [ -z $blockHeight ] || [ "$blockHeight" -eq 0 ]; then
    ...
    do  #catch up from previous to current blockheight
      # blockHash=$(grep -m 1 "$iblockHeight" ${logfile[ -z $blockHeight ] || [ "$blockHeight" -eq 0 ]} | jq -r .data.block)
      blockHash=$(grep -m 1 "$iblockHeight" ${logfile} | jq -r .data.block)
      if [ ! -z $blockHash ]; then
